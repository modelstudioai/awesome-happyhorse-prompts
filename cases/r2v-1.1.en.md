# R2V Reference-to-Video Cases (HappyHorse 1.1)

🌐 **Language:** English · [🇨🇳 中文](r2v-1.1.md)

> HappyHorse 1.1 R2V dramatically strengthens reference-image attention. Highlights: 3×3 storyboard support, multiple character references on-screen without cross-contamination, separate inputs for character vs. scene references. A single prompt can describe 6–8 sequential shots; the model schedules cuts and preserves identity across them.

**When to use:**
- Generate continuous ads / animations / shorts panel-by-panel from a 3×3 storyboard.
- Multi-character framing with consistent appearance, wardrobe and demeanour.
- Separate scene and character references — same character across changing scenes.
- Live-commerce, brand ads, anywhere a fixed product / persona must be preserved.

**Prompt patterns:**
- `[Image 1]` is a 3×3 storyboard reference; generate panels strictly in left-to-right, top-to-bottom order.
- `[Image 1]` = scene reference, `[Image 2]` = man, `[Image 3]` = woman; render a hotel-suite night dialogue.
- `[Image 1]` = opening livestream frame, `[Image 2]` = try-on, `[Image 3]` = product flat-lay, `[Image 4]` = home interior.

---

### Case 1: Convenience Store Healing Night — K-drama 3×3 storyboard

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** Drive a 9-shot K-drama-style sequence from a single 3×3 storyboard image. Levers worth tuning when adapting: panel reading order (locked left-to-right, top-to-bottom), tonal contrast (warm interior vs. cold rainy night), the global "no on-screen text" guard, per-panel framing keywords (wide / medium / close-up / extreme close-up), and the embedded Korean line `오늘도 수고 많았어요.` — kept verbatim because translating it removes the cultural marker and the lip-sync test.
...
| Reference 1 |
|:---:|
| <img src="../videos/r2v-1.1/images/ref_storyboard_convenience.png" width="300" alt="Reference 1"> |
...
### Case 2: Girl and Cat — Japanese animation cinematic storyboard

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** A 9-shot summer-rural Japanese animation piece driven from a 3×3 board. Levers: explicit "no text, no grid lines" guard (`画面严禁出现任何文字或拼图网格线`), character spec (yellow dress, black short hair) + companion spec (grey tabby kitten) + hero prop (the wind chime under the eaves), plus per-panel composition cues that vary framing distance. Tone keywords (`温暖阳光、清新治愈、安静怀旧`) lock the colour and pacing across panels.
...
| Reference 1 |
|:---:|
| <img src="../videos/r2v-1.1/images/girlcats.png" width="300" alt="Reference 1"> |
...
### Case 3: Happy Run Citrus — Japanese youth sports-drink commercial

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** A 9-panel J-CM (Japanese commercial) for a fictional sports drink. The prompt mixes Mandarin direction with embedded Japanese voice-over lines (`「気持ちに、点火しよう。」`, `「走り出せば、世界が変わる。」`, `「集中、フルチャージ。」`, `「一緒なら、もっと上へ！」`, `「明日の、加速を。」`) — match prompt voice-over language to the cultural target. Levers to tune: brand-color contrast lock (orange + blue), wardrobe spec, the per-panel narration assigned to each shot, the explicit "convert in-board Japanese ad copy into VO/expression instead of on-screen subtitles" rule (`故事板里的日文广告语不要强行生成成画面字幕，而是转化为旁白或情绪表达，避免视频里出现乱码文字`). Brand "Happy Run" is fictional — swap in your own SKU for production use.
...
| Reference 1 |
|:---:|
| <img src="../videos/r2v-1.1/images/happyrunv3.png" width="300" alt="Reference 1"> |
...
### Case 4: STAR Fashion — luxury fashion commercial

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** A 15-second luxury commercial driven by a 3×3 board, fully prompted in English. Note the structural pattern: a `[Style]` block (look, lens, palette, reference benchmark "Chanel-level"), `[Character]` and `[Products]` blocks for identity locks, then a numbered `[Shot Sequence]` with one English sentence per shot. Levers worth tuning: the editorial rhythm directive `fast-paced editorial rhythm`, the photoreal-not-illustration guard (`Photorealistic luxury fashion-ad cinematography. NOT illustration, NOT anime`), and the explicit consistency lock `Maintain identical appearance across all shots`. Good template when you want a high-end look without timecodes.
...
| Reference 1 |
|:---:|
| <img src="../videos/r2v-1.1/images/commercialad.png" width="300" alt="Reference 1"> |
...
## Multi-character / object × scene references

### Case 5: Live Commerce — multi-image-referenced pants showcase

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** A vertical live-commerce video built from four references: opening live frame, try-on transformation, the product flat-lay, and the home environment. Note the prompt is bilingual on purpose — the **English block is the canonical input** (because the host's spoken lines are in English) and the trailing `> 中文 Prompt 参考：` block is provided as a translation aid; pick one for the actual call. Levers to tune: timecode block `[0-4s]` / `[4-5s]` / `[5-10s]` / `[10-13s]` / `[13-15s]`, the "whip-pan transition" beat, fabric-hand cues (`pinches the fabric near the waistband, stretching it to show softness`), and the on-screen-clean-of-UI guard (`No subtitles, no text, no watermark, no brand logo, no shopping UI`).
...
|:---:|:---:|:---:|:---:|
| <img src="../videos/r2v-1.1/images/livestream_image1.png" width="300" alt="Reference 1"> | <img src="../videos/r2v-1.1/images/livestream_image2.png" width="300" alt="Reference 2"> | <img src="../videos/r2v-1.1/images/livestream_image3.png" width="300" alt="Reference 3"> | <img src="../videos/r2v-1.1/images/livestream_image4.png" width="300" alt="Reference 4"> |
...
### Case 6: Hotel Suite — two-character emotional drama (sliced R2V)

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** Two consecutive ~15s clips chained for a hotel-suite two-hander. Note the **slicing pattern**: instead of trying to fit a 30-second drama in one prompt, the spec splits into `**Part1**` and `**Part2**`, each with its own scene anchor (Image 1) and character anchors (Image 2 / Image 3). Levers to tune: per-segment timecode `[00:00-00:07]` / `[00:07-00:15]`, the props that act as memory across cuts (whisky glass, black gift bag, phone face-down), and the cinematography lock at the end of each part (`电影级浅景深，低饱和冷暖双色调，微胶片颗粒，16:9画幅`). When stitching the two clips post-render, an `xfade` cross-dissolve gives the cleanest emotional handoff.
...
|:---:|:---:|:---:|
| <img src="../videos/r2v-1.1/images/hotel_image1.png" width="300" alt="Reference 1"> | <img src="../videos/r2v-1.1/images/hotel_image2.png" width="300" alt="Reference 2"> | <img src="../videos/r2v-1.1/images/hotel_image3.png" width="300" alt="Reference 3"> |
...
### Case 7: Song-dynasty Study — period-drama emotional short (sliced R2V)

**Model:** `happyhorse-1.1-r2v`

> **Prompt intent (EN annotation):** Three chained R2V clips (CLIP1 12s + CLIP2 8s + CLIP3 8s) for a Song-dynasty period-drama study scene. This is the most demanding case in the suite — note the **rigorous spatial anchor block** repeated in CLIP2 and CLIP3 (`空间布局（必须严格遵守）：书案横置画面中央，男人坐书案后方正中（面朝镜头），女人站书案左后方约0.5米…`). 1.1's stronger spatial reasoning lets you carry blocking across cuts, but only if the prompt restates it. Levers to tune: lighting direction lock (`暖烛光从右、冷月光从左后`), the "no readable Chinese characters on paper, only blurred Song-style brushwork" guard (`所有纸面文字必须是宋代竖排毛笔行书风格的模糊背景质感，浅景深柔焦处理，不出现可清晰辨读的整段汉字`), the per-line speaker/emotion tags (`[男人/低声/疲惫克制/"…"]`), and the explicit "every line must be fully voiced, no shortening" rule (`每句台词必须完整念出不省略`).
...
| Reference 1 | Reference 2 | Reference 3 |
|:---:|:---:|:---:|
| <img src="../videos/r2v-1.1/images/song_image1.png" width="300" alt="Reference 1"> | <img src="../videos/r2v-1.1/images/song_image2.png" width="300" alt="Reference 2"> | <img src="../videos/r2v-1.1/images/song_image3.png" width="300" alt="Reference 3"> |
...
## R2V 1.1 authoring tips

- `[Image N]` **requires the space** between `Image` and the number — hard requirement of the API parser.
- For multi-character setups, the recommended convention is `[Image 1]` = scene, `[Image 2]` = character A, `[Image 3]` = character B.
- For a 3×3 storyboard, describe panels in reading order; the model allocates per-shot duration automatically.
- Append global constraints at the end of the prompt: text suppression, identity consistency, lip-sync.
- 1.1 follows timecodes more literally than 1.0 — for performance / emotional scenes, you can loosen them.
